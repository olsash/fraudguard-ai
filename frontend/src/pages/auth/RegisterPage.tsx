...
export default function Register() {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const result = await authService.register(fullName, email, pwd);
    ...
    <AuthShell title="Create your account" subtitle="Start detecting fraud in under 2 minutes">
      <form className="space-y-4" onSubmit={handleSubmit}>
        <div className="grid grid-cols-2 gap-3">
          <Field label="First name" placeholder="Sara" value={firstName} onChange={(event) => setFirstName(event.target.value)} required />
          <Field label="Last name" placeholder="Amrani" value={lastName} onChange={(event) => setLastName(event.target.value)} required />
        </div>
        ...
        <label className="block">
          ...
          <div className="mt-1 flex items-center glass rounded-lg px-3 py-2.5">
            <input type="password" value={pwd} onChange={(event) => setPwd(event.target.value)} placeholder="••••••••" className="flex-1 bg-transparent text-sm outline-none" required minLength={6} />
          </div>
